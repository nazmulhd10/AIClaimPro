...
# import uuid
# from django.contrib.auth.decorators import login_required
# from claims.models import UserProfile, Claim
...
#         user_profile = UserProfile.objects.get(user=request.user)
#     except UserProfile.DoesNotExist:
#         # Generate a unique imid
#         while True:
#             new_imid = str(uuid.uuid4())
#             if not UserProfile.objects.filter(imid=new_imid).exists():
#                 break
#         user_profile = UserProfile.objects.create(
#             user=request.user,
#             imid=new_imid,
#             phone_number=""  # Set default or fetch from elsewhere if needed
#         )
#     claims = Claim.objects.filter(user=request.user).order_by('-submission_date')
#     return render(request, 'claims/dashboard.html', {'user_profile': user_profile, 'claims': claims})
...
import json
import traceback
import uuid  # Added for generating unique imid
from django.http import JsonResponse
from django.shortcuts import render, redirect, get_object_or_404
import requests
...
def dashboard(request):
    user_profile, created = UserProfile.objects.get_or_create(
        user=request.user,
        defaults={'imid': str(uuid.uuid4()), 'phone_number': ''}  # Ensure unique imid
    )
    claims = Claim.objects.filter(user=request.user).order_by('-submission_date')
    return render(request, 'claims/dashboard.html', {'user_profile': user_profile, 'claims': claims})
...
def submit_claim_contract_documents(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                # Redirect to the same page after successful submission
                return redirect('submit_claim_contract_documents')
            except Exception as e:
                print("💥 Exception occurred while saving contract document:", e)
        else:
            print("❌ Form invalid:", form.errors)
    ...
    # Load user-specific submitted data
    ClaimContractDocuments = ClaimContractDocument.objects.filter(user=request.user).order_by('-submission_date')
    ...
    return render(
        ...
        {
            'form': form,
            'ClaimContractDocuments': ClaimContractDocuments
        }
    )
